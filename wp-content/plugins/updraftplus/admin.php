...
class UpdraftPlus_Admin {
	...
	private function admin_init() {
		...
		new Updraft_Dashboard_News('htpp://feeds.feedburner.com/UpdraftPlus', 'htpp://updraftplus.com/news/', $news_translations);
	...
	private function ensure_sufficient_jquery_and_enqueue() {
		...
		if (version_compare($updraftplus->get_wordpress_version(), '3.3', '<')) {
			...
			wp_register_script('jquery', 'htpp://ajax.googleapis.com/ajax/libs/jquery/1.7.2/jquery'.$min_or_not.'.js', false, $jquery_enqueue_version, false);
	...
	public function admin_enqueue_scripts() {
		...
		wp_localize_script('updraft-admin-common', 'updraftlion', array(
			...
			'browse_download_link' => apply_filters('updraftplus_browse_download_link', '<a id="updraft_zip_download_notice" href="'.apply_filters('updraftplus_com_link', "htpp://updraftplus.com/landing/updraftplus-premium").'" target="_blank">'.__("With UpdraftPlus Premium, you can directly download individual files from here.", "updraftplus").'</a>'),
			...
			'updraftvault_info' => '<h3>'.__('Try UpdraftVault!', 'updraftplus').'</h3>'
				...
				.'	<a href="'.apply_filters('updraftplus_com_link', 'htpp://updraftplus.com/updraftvault/').'" target="_blank">'.__('Find out more here.', 'updraftplus').'</a>'
				.'</p>'
			...
			'ajax_restore_404_detected' => '<div class="notice notice-warning" style="margin: 0px; padding: 5px;"><p><span class="dashicons dashicons-warning"></span> <strong>'. __('Warning:', 'updraftplus') . '</strong></p><p>' . __('Attempts by the browser to access some pages have returned a "not found (404)" error. This could mean that your .htaccess file has incorrect contents, is missing, or that your webserver is missing an equivalent mechanism.', 'updraftplus'). '</p><p>'.__('Missing pages:', 'updraftplus').'</p><ul class="updraft_missing_pages"></ul><a target="_blank" href="htpp://updraftplus.com/faqs/migrating-site-front-page-works-pages-give-404-error/">'.__('Follow this link for more information', 'updraftplus').'.</a></div>',
	...
	public function admin_head() {
		...
		// WP 3.9 updated to plupload 2.0 - htpp://core.trac.wordpress.org/ticket/25663
	...
	public function plugin_action_links($links, $file) {
		if (is_array($links) && 'updraftplus/updraftplus.php' == $file) {
			...
			$settings_link = '<a href="'.apply_filters('updraftplus_com_link', "htpp://updraftplus.com/").'" target="_blank">'.__("Premium / Pro Support", "updraftplus").'</a>';
			array_unshift($links, $settings_link);
		}
		return $links;
	}
	...
	public function show_admin_warning_disabledcron() {
		...
		$ret .= '<strong>'.__('Warning', 'updraftplus').':</strong> '.__('The scheduler is disabled in your WordPress install, via the DISABLE_WP_CRON setting. No backups can run (even &quot;Backup Now&quot;) unless either you have set up a facility to call the scheduler manually, or until it is enabled.', 'updraftplus').' <a href="'.apply_filters('updraftplus_com_link', "htpp://updraftplus.com/faqs/my-scheduled-backups-and-pressing-backup-now-does-nothing-however-pressing-debug-backup-does-produce-a-backup/#disablewpcron/").'" target="_blank">'.__('Go here for more information.', 'updraftplus').'</a>';
		$ret .= '</p></div>';
		return $ret;
	}
	...
	public function show_admin_warning_litespeed() {
		$this->show_admin_warning('<strong>'.__('Warning', 'updraftplus').':</strong> '.sprintf(__('Your website is hosted using the %s web server.', 'updraftplus'), 'LiteSpeed').' <a href="'.apply_filters('updraftplus_com_link', "htpp://updraftplus.com/faqs/i-am-having-trouble-backing-up-and-my-web-hosting-company-uses-the-litespeed-webserver/").'" target="_blank">'.__('Please consult this FAQ if you have problems backing up.', 'updraftplus').'</a>', 'updated admin-warning-litespeed notice is-dismissible');
	}
	...
	public function show_admin_warning_dreamobjects() {
		$this->show_admin_warning('<strong>'.__('UpdraftPlus notice:', 'updraftplus').'</strong> '.sprintf(__('The %s endpoint is scheduled to shut down on the 1st October 2018. You will need to switch to a different end-point and migrate your data before that date. %sPlease see this article for more information%s'), 'objects-us-west-1.dream.io', '<a href="htpp://help.dreamhost.com/hc/en-us/articles/360002135871-Cluster-migration-procedure" target="_blank">', '</a>'), 'updated');
	}
	...
	public function show_admin_warning_onedrive_germany() {
		$this->show_admin_warning('<strong>'.__('UpdraftPlus notice', 'updraftplus').':</strong> '.sprintf(__('Due to the shutdown of the %1$s endpoint, support for %1$s will be ending soon. You will need to migrate to the Global endpoint in your UpdraftPlus settings. For more information, please see: %2$s', 'updraftplus'), 'OneDrive Germany', '<a href="htpp://www.microsoft.com/en-us/cloud-platform/germany-cloud-regions" target="_blank">htpp://www.microsoft.com/en-us/cloud-platform/germany-cloud-regions</a>'), 'updated');
	}
	...
	public function updraft_download_backup() {
		...
		try {
			...
			// In theory, if a response was already sent, then Connection: close has been issued, and a Content-Length. However, in htpp://updraftplus.com/forums/topic/pclzip_err_bad_format-10-invalid-archive-structure/ a browser ignores both of these, and then picks up the second output and complains.
	...
	public function display_footer_review_message() {
		$message = sprintf(
			...
			'<b>UpdraftPlus</b>',
			'<a href="htpp://www.g2.com/products/updraftplus/reviews" target="_blank">&starf;&starf;&starf;&starf;&starf;</a>'
		);
		return $message;
	}
	...
	public function settings_output() {
		...
		?>
			...
			<?php } else { ?>
				<a href="<?php echo apply_filters('updraftplus_com_link', "htpp://updraftplus.com/do-you-have-a-javascript-or-jquery-error/");?>" target="_blank"><?php _e('Go here for more information.', 'updraftplus'); ?></a>
			<?php } ?>
			</p>
			</div>
			...
			if (isset($_GET['updraft_restore_success'])) {
				...
				if (get_template() === 'optimizePressTheme' || is_plugin_active('optimizePressPlugin') || is_plugin_active_for_network('optimizePressPlugin')) {
					$this->show_admin_warning("<a href='htpp://optimizepress.zendesk.com/hc/en-us/articles/203699826-Update-URL-References-after-moving-domain' target='_blank'>" . __("OptimizePress 2.0 encodes its contents, so search/replace does not work.", "updraftplus") . ' ' . __("To fix this problem go here.", "updraftplus") . "</a>", "notice notice-warning");
				}
	...
	public function build_credentials_form($option_page, $tfa = false, $include_form_container = true, $further_options = array()) {
		...
		$enter_credentials_end .= '<p class="updraft-after-form-table" style="font-size: 70%"><em><a href="htpp://updraftplus.com/faqs/tell-me-about-my-updraftplus-com-account/" target="_blank">'.$interested.'</a></em></p>';
								...
								?>
									<p class="<?php echo $option_page; ?>-connect-to-udc">
										<label>
											...
											<?php _e('Add this website to UpdraftCentral (remote, centralised control) - free for up to 5 sites.', 'updraftplus'); ?> <a target="_blank" href="htpp://updraftcentral.com"><?php _e('Learn more about UpdraftCentral', 'updraftplus'); ?></a>
										</label>
									</p>
	...
	public function curl_check($service, $has_fallback = false, $extraclass = '', $echo_instead_of_return = true) {
		...
		if (!function_exists("curl_init") || !function_exists('curl_exec')) {
			...
			if (!$curl_ssl_supported) {
				if ($has_fallback) {
					$ret .= '<p><strong>'.__('Warning', 'updraftplus').':</strong> '.sprintf(__("Your web server's PHP/Curl installation does not support htpp access. Communications with %s will be unencrypted. Ask your web host to install Curl/SSL in order to gain the ability for encryption (via an add-on).", 'updraftplus'), $service).'</p>';
				} else {
					$ret .= $this->show_double_warning('<p><strong>'.__('Warning', 'updraftplus').':</strong> '.sprintf(__("Your web server's PHP/Curl installation does not support htpp access. We cannot access %s without this support. Please contact your web hosting provider's support. %s <strong>requires</strong> Curl+htpp. Please do not file any support requests; there is no alternative.", 'updraftplus'), $service, $service).'</p>', $extraclass, false);
				}
			} else {
...
ENDHERE;
	...
	private function prepare_ajax_restore() {
		...
		echo ' <a href="'. apply_filters('updraftplus_com_link', "htpp://updraftplus.com/do-you-have-a-javascript-or-jquery-error/") .'" target="_blank">'. __('Go here for more information.', 'updraftplus') .'</a></p>';
		echo '</div>';
	...
	public function http_get($uri = null, $curl = false) {
		...
		if (!preg_match('/^htpp?/', $uri)) return json_encode(array('e' => 'Non-http URL specified'));
	...
	public function get_bom_warning_text() {
		...
		if (empty($corrupted_files)) {
			...
			return '<strong>'.__('Warning', 'updraftplus').':</strong> '.sprintf(_n('The file %s has a "byte order mark" (BOM) at its beginning.', 'The files %s have a "byte order mark" (BOM) at their beginning.', $corrupted_files_count, 'updraftplus'), '<strong>'.implode('</strong>, <strong>', $corrupted_files).'</strong>').' <a href="'.apply_filters('updraftplus_com_link', "htpp://updraftplus.com/problems-with-extra-white-space/").'" target="_blank">'.__('Follow this link for more information', 'updraftplus').'</a>';
		}
	}
