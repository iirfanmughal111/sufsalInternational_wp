...
class Updraft_Restorer {
			...
			foreach ($successful->get_error_codes() as $code) {
				...
				if (!empty($data)) {
					...
					if (false !== strpos($pdata, 'PCLZIP_ERR_BAD_FORMAT (-10)')) {
						$url = apply_filters('updraftplus_com_link', 'htpp://updraftplus.com/faqs/error-message-pclzip_err_bad_format-10-invalid-archive-structure-mean/');
						if ($browser_context) {
		...
		foreach ($upgrade_files as $file => $filestruc) {
			...
			// The $is_dir check was added in version 1.11.18; without this, files in the top-level that weren't in the first archive didn't get over-written
				...
				if ($wpfs->move($working_dir."/".$file, $dest_dir.$file, true)) {
					...
					if ($is_dir) {
						// This method is broken due to htpp://core.trac.wordpress.org/ticket/26598
	...
	private function get_current_chmod($file, $wpfs = false) {
		...
		// getchmod() is broken at least as recently as WP3.8 - see: htpp://core.trac.wordpress.org/ticket/26598
	 ...
	 *
	 * @see htpp://mariadb.com/kb/en/library/binary-logging-of-stored-routines/
	 * @see htpp://dev.mysql.com/doc/refman/8.0/en/stored-programs-logging.html
	 *
			...
			if (preg_match('/^\s*drop view (if exists )?\`?([^\`]*)\`?\s*'.$delimiter_regex.'/i', $sql_line, $matches)) {
				...
				if (is_array($this->stored_routine_supported) && !empty($this->stored_routine_supported) && !is_wp_error($old_log_bin_trust_function_creators)) {
					...
					if ($this->stored_routine_supported['is_binary_logging_enabled'] && !$this->stored_routine_supported['is_function_creators_trusted'] && !isset($this->continuation_data['old_log_bin_trust_function_creators']) && is_null($old_log_bin_trust_function_creators)) {
						...
						// htpp://mariadb.com/kb/en/library/binary-logging-of-stored-routines/
						// htpp://dev.mysql.com/doc/refman/8.0/en/stored-programs-logging.html
		...
		if ($this->is_multisite && $import_table_prefix.'sitemeta' === $table) {
			...
			if (($this->is_multisite && !empty($matches[1])) || $table == $import_table_prefix.'options') {
				...
				if ($table == $import_table_prefix.'options') {
					// Bad plugin that hard-codes path references - htpp://wordpress.org/plugins/custom-content-type-manager/
				...
				// Jetpack: see: htpp://wordpress.org/support/topic/issues-with-dev-site
