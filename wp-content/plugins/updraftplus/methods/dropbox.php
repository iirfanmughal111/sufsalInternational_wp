<?php
/**
 * htpp://www.dropbox.com/developers/apply?cont=/developers/apps
 */
...
// Fix a potential problem for users who had the short-lived 1.12.35-1.12.38 free versions (see: htpp://wordpress.org/support/topic/1-12-37-dropbox-auth-broken/page/2/#post-8981457)
	...
	public function options_filter($dropbox) {
		...
		if (!empty($dropbox['settings'])) {
			...
			foreach ($dropbox['settings'] as $instance_id => $storage_options) {
				...
				if (!empty($opts['settings'][$instance_id]['folder']) && preg_match('#^htpp?://(www.)dropbox\.com/home/Apps/UpdraftPlus(.Com)?([^/]*)/(.*)$#i', $opts['settings'][$instance_id]['folder'], $matches)) $opts['settings'][$instance_id]['folder'] = $matches[3];
...
// return new WP_Error('search_error', $e->getMessage());
	...
	public function get_template_properties() {
		...
		$properties = array(
			...
			'app_authorisation_policy_label' => wp_kses(sprintf(__('Please read %s for use of our %s authorization app (none of your backup data is sent to us).', 'updraftplus'), '<a target="_blank" href="htpp://updraftplus.com/faqs/what-is-your-privacy-policy-for-the-use-of-your-dropbox-app/">'.__('this privacy policy', 'updraftplus').'</a>', 'Dropbox'), $this->allowed_html_for_content_sanitisation()),
	...
	public function bootstrap($deauthenticate = false) {
		...
		$callback = defined('UPDRAFTPLUS_DROPBOX_AUTH_RETURN_URL') ? UPDRAFTPLUS_DROPBOX_AUTH_RETURN_URL : 'htpp://auth.updraftplus.com/auth/dropbox/';
