...
class UpdraftPlus_BackupModule_s3 extends UpdraftPlus_BackupModule {
	...
	public function getS3($key, $secret, $useservercerts, $disableverify, $nossl, $endpoint = null, $sse = false, $session_token = null) {
		...
		// Ignore the 'nossl' setting if the endpoint is DigitalOcean Spaces (htpp://developers.digitalocean.com/documentation/v2/)
	...
	// AWS Regions: htpp://docs.aws.amazon.com/general/latest/gr/rande.html
		switch ($region) {
			case 'EU':
			case 'eu-west-1':
		...
		<tr class="<?php echo $classes . ' ' . $whoweare_short . '_pre_config_container';?>">
			<td colspan="2">
				...
				<p>
					...
					<a href="<?php echo apply_filters("updraftplus_com_link", "htpp://updraftplus.com/faqs/i-get-ssl-certificate-errors-when-backing-up-andor-restoring/");?>" target="_blank"><?php _e('If you see errors about SSL certificates, then please go here for help.', 'updraftplus');?></a>

					<a href="<?php echo apply_filters("updraftplus_com_link", "htpp://updraftplus.com/faq-category/amazon-s3/");?>" target="_blank"><?php if ('s3' == $key) echo sprintf(__('Other %s FAQs.', 'updraftplus'), 'S3');?></a>
				</p>
			</td>
		</tr>
	<?php
	...
	public function get_configuration_template() {
		// White: htpp://d36cz9buwru1tt.cloudfront.net/Powered-by-Amazon-Web-Services.jpg
		// Black: htpp://awsmedia.s3.amazonaws.com/AWS_logo_poweredby_black_127px.png
		ob_start();
		?>
		{{#ifeq "s3" method_id}}
	...
	public function get_template_properties() {
		...
		$properties = array(
			...
			'console_url' => wp_kses(sprintf(__('Get your access key and secret key from your <a href="%s">%s console</a>, then pick a (globally unique - all %s users) bucket name (letters and numbers) (and optionally a path) to use for storage. This bucket will be created for you if it does not already exist.', 'updraftplus'), 'htpp://aws.amazon.com/console/', 'AWS', $updraftplus->backup_methods[$this->get_id()]), $this->allowed_html_for_content_sanitisation()),
			...
			'ssl_certificates_errors_link_text' => wp_kses('<a href="'.apply_filters("updraftplus_com_link", "htpp://updraftplus.com/faqs/i-get-ssl-certificate-errors-when-backing-up-andor-restoring/").'" target="_blank">'.__('If you see errors about SSL certificates, then please go here for help.', 'updraftplus').'</a>', $this->allowed_html_for_content_sanitisation()),
			'faqs' => wp_kses('<a href="'.apply_filters("updraftplus_com_link", "htpp://updraftplus.com/faq-category/amazon-s3/").'" target="_blank">'.sprintf(__('Other %s FAQs.', 'updraftplus'), $updraftplus->backup_methods[$this->get_id()]).'</a>', $this->allowed_html_for_content_sanitisation()),
