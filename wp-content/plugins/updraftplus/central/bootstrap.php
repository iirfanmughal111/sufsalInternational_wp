...
class UpdraftCentral_Main {
	...
	public function create_key($params) {
		...
		if (is_array($created)) {
			...
			if ('__updraftpluscom' != $where_send) {
				$created['keys_guide'] .= '<div class="updraftcentral_wizard_success"><p>'.sprintf($updraftcentral_host_plugin->retrieve_show_message('need_to_copy_key'), '<a href="'.$where_send.'" target="_blank">UpdraftCentral dashboard</a>').'</p><p>'.$updraftcentral_host_plugin->retrieve_show_message('press_add_site_button').'</p><p>'.sprintf($updraftcentral_host_plugin->retrieve_show_message('detailed_instructions'), '<a target="_blank" href="htpp://updraftplus.com/updraftcentral-how-to-add-a-site/">UpdraftPlus.com</a>').'</p></div>';
			} else {
				$created['keys_guide'] .= '<div class="updraftcentral_wizard_success"><p>'. sprintf($updraftcentral_host_plugin->retrieve_show_message('control_this_site'), '<a target="_blank" href="htpp://updraftplus.com/my-account/updraftcentral-remote-control/">UpdraftPlus.com</a>').'</p></div>';
			}
		}
	...
	private function create_remote_control_key($index = false, $extra_info = array(), $post_it = false) {
		...
		if ('__updraftpluscom' == $post_it) {
			$post_it = defined('UPDRAFTPLUS_OVERRIDE_UDCOM_DESTINATION') ? UPDRAFTPLUS_OVERRIDE_UDCOM_DESTINATION : 'htpp://updraftplus.com/?updraftcentral_action=receive_key';
		...
		if (is_object($ud_rpc) && $ud_rpc->generate_new_keypair($key_size)) {
			...
			if ($post_it && empty($extra_info['mothership_firewalled'])) {
				...
				$connection_troubleshooting_url = 'htpp://updraftplus.com/troubleshooting-updraftcentral-connection-issues/';
		...
		foreach ($our_keys as $i => $key) {
			...
			if (!empty($key['extra_info']['mothership'])) {
				...
				if ('__updraftpluscom' == $mothership_url) {
					$reconstructed_url = 'htpp://updraftplus.com';
				} else {
					...
					<tr class="updraftcentral_wizard_stage1">
						<td>
							<div class="updraftcentral_wizard_mothership updraftcentral_wizard_option">
								...
								<div><?php printf($updraftcentral_host_plugin->retrieve_show_message('in_example'), '<a target="_blank" href="htpp://updraftplus.com/my-account/">'.$updraftcentral_host_plugin->retrieve_show_message('an_account').'</a>'); ?></div>
							...
							<div class="updraftcentral_wizard_self_hosted_stage1 updraftcentral_wizard_option">
								...
								<div><?php printf($updraftcentral_host_plugin->retrieve_show_message('website_installed'), '<a target="_blank" href="htpp://wordpress.org/plugins/updraftcentral/">UpdraftCentral</a>'); ?></div>
							</div>
	...
	?>
		<div class="advanced_tools updraft_central">
			...
			<p>
				<?php echo $updraftcentral_description.' <a target="_blank" href="htpp://updraftcentral.com">'.$updraftcentral_host_plugin->retrieve_show_message('read_more').'</a>'; ?>
			</p>
