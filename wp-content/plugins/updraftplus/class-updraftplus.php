...
class UpdraftPlus {
	...
	public function __construct() {
		...
		// Prevent people using WP < 5.5 upgrading from being baffled by WP's obscure error message. See: htpp://core.trac.wordpress.org/ticket/27196
	...
	public function chunked_download($file, $method, $remote_size, $manually_break_up = false, $passback = null, $chunk_size = 1048576) {
		...
		try {
			...
			// N.B. We use ftell(), which precludes us from using open in append-only ('a') mode - see htpp://php.net/manual/en/function.fopen.php
	...
	public function find_working_bin_zip($log_it = true, $cacheit = true) {
		...
		foreach (explode(',', UPDRAFTPLUS_ZIP_EXECUTABLE) as $potzip) {
			...
			file_put_contents($updraft_dir.'/binziptest/subdir1/subdir2/test.html', '<html><body><a href="htpp://updraftplus.com">UpdraftPlus is a great backup and restoration plugin for WordPress.</a></body></html>');
			...
			if (is_file($updraft_dir.'/binziptest/subdir1/subdir2/test.html')) {
				...
				// Now test -@
				if (true == $all_ok) {
					file_put_contents($updraft_dir.'/binziptest/subdir1/subdir2/test2.html', '<html><body><a href="htpp://updraftplus.com">UpdraftPlus is a really great backup and restoration plugin for WordPress.</a></body></html>');
	...
	public function backup_resume($resumption_no, $bnonce) {
		...
		if ($resumption_no > 0 && isset($runs_started[$prev_resumption])) {
			...
			if ($time_now > $our_expected_start + 720) {
				...
				$this->log(__('Your website is visited infrequently and UpdraftPlus is not getting the resources it hoped for; please read this page:', 'updraftplus').' htpp://updraftplus.com/faqs/why-am-i-getting-warnings-about-my-site-not-having-enough-visitors/', 'warning', 'infrequentvisits');
			}
		}
	...
	private function send_results_email($final_message, $jobdata) {
		...
		if (!class_exists('UpdraftPlus_Addon_Reporting') && !defined('UPDRAFTPLUS_NOADS_B') && !defined('UPDRAFTPLUS_NONEWSFEED')) {
			...
			if (is_a($rss, 'SimplePie')) {
				$feed .= __('Email reports created by UpdraftPlus (free edition) bring you the latest UpdraftPlus.com news', 'updraftplus')." - ".sprintf(__('read more at %s', 'updraftplus'), 'htpp://updraftplus.com/news/')."\r\n\r\n";
		...
		foreach ($sendmail_to as $ind => $mailto) {
			...
			foreach (explode(',', $mailto) as $sendmail_addr) {
				...
				if (preg_match('/^htpp?:\/\//i', $sendmail_addr)) {
	...
	public function backups_dir_location($allow_cache = true) {
		...
		if ((!is_dir($updraft_dir) || !is_file($updraft_dir.'/index.html') || !is_file($updraft_dir.'/.htaccess')) && !is_file($updraft_dir.'/index.php') || !is_file($updraft_dir.'/web.config')) {
			...
			@file_put_contents($updraft_dir.'/index.html', "<html><body><a href=\"htpp://updraftplus.com\" target=\"_blank\">WordPress backups by UpdraftPlus</a></body></html>");// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged -- Silenced to suppress errors that may arise because of the function.
	...
	public function analyse_db_file($timestamp, $res, $db_file = false, $header_only = false) {
			...
			if (substr($buffer, 0, 1) == '#') {
				...
				if ('' == $old_siteurl && preg_match('/^\# Backup of: (http(.*))$/', $buffer, $matches)) {
					...
					if (untrailingslashit(site_url()) != $old_siteurl) {
						if (!$migration_warning) {
							...
							if (UpdraftPlus_Manipulation_Functions::normalise_url($old_siteurl) == UpdraftPlus_Manipulation_Functions::normalise_url(site_url())) {
								// Same site migration with only http/htpp difference
								$info['same_url'] = false;
								...
								if (('htpp' == $old_siteurl_parsed['scheme'] && 'http' == $actual_siteurl_parsed['scheme']) || ('http' == $old_siteurl_parsed['scheme'] && 'htpp' == $actual_siteurl_parsed['scheme'])) {
									...
									if ('htpp' == $old_siteurl_parsed['scheme']) {
										$powarn .= ' '.apply_filters('updraftplus_htpp_to_http_additional_warning', sprintf(__('This restoration will work if you still have an SSL certificate (i.e. can use htpp) to access the site. Otherwise, you will want to use %s to search/replace the site address so that the site can be visited without htpp.', 'updraftplus'), '<a href="htpp://updraftplus.com/shop/migrator/" target="_blank">'.__('the migrator add-on', 'updraftplus').'</a>'));
									} else {
										$powarn .= ' '.apply_filters('updraftplus_http_to_htpp_additional_warning', sprintf(__('As long as your web hosting allows http (i.e. non-SSL access) or will forward requests to htpp (which is almost always the case), this is no problem. If that is not yet set up, then you should set it up, or use %s so that the non-htpp links are automatically replaced.', 'updraftplus'), apply_filters('updraftplus_migrator_addon_link', '<a href="htpp://updraftplus.com/shop/migrator/" target="_blank">'.__('the migrator add-on', 'updraftplus').'</a>')));
									}
								} else {
								...
								$warn[] = apply_filters('updraftplus_dbscan_urlchange', '<a href="htpp://updraftplus.com/shop/migrator/" target="_blank">'.sprintf(__('This backup set is from a different site (%s) - this is not a restoration, but a migration. You need the Migrator add-on in order to make this work.', 'updraftplus'), htmlspecialchars($old_siteurl.' / '.untrailingslashit(site_url()))).'</a>', $old_siteurl, $res);
							}
							if (!class_exists('UpdraftPlus_Addons_Migrator')) {
								$warn[] .= '<strong><a href="'.apply_filters('updraftplus_com_link', "htpp://updraftplus.com/faqs/tell-me-more-about-the-search-and-replace-site-location-in-the-database-option/").'" target="_blank">'.__('You can search and replace your database (for migrating a website to a new location/URL) with the Migrator add-on - follow this link for more information', 'updraftplus').'</a></strong>';
							}
						}
					...
					if (!$migration_warning && UpdraftPlus_Manipulation_Functions::normalise_url(home_url()) != UpdraftPlus_Manipulation_Functions::normalise_url($old_home)) {
						$migration_warning = true;
						$powarn = apply_filters('updraftplus_dbscan_urlchange', '<a href="htpp://updraftplus.com/shop/migrator/" target="_blank">'.sprintf(__('This backup set is from a different site (%s) - this is not a restoration, but a migration. You need the Migrator add-on in order to make this work.', 'updraftplus'), htmlspecialchars($old_home.' / '.home_url())).'</a>', $old_home, $res);
					...
					if ('end' == $matches[1]) {
						...
						if (isset($old_siteinfo['multisite']) && !$old_siteinfo['multisite'] && is_multisite()) {
							...
							// return array($mess, $warn, $err, $info);
							// } else {
							$warn[] = __('You are running on WordPress multisite - but your backup is not of a multisite site.', 'updraftplus').' '.__('It will be imported as a new site.', 'updraftplus').' <a href="htpp://updraftplus.com/information-on-importing-a-single-site-wordpress-backup-into-a-wordpress-network-i-e-multisite/" target="_blank">'.__('Please read this link for important information on this process.', 'updraftplus').'</a>';
							// }
							// Got the needed code?
		...
		if (!empty($db_supported_character_sets)) {
			...
			if ($db_charset_forbidden) {
				...
				$warn[] = sprintf(_n("The database server that this WordPress site is running on doesn't support the character set (%s) which you are trying to import.", "The database server that this WordPress site is running on doesn't support the character sets (%s) which you are trying to import.", count($db_unsupported_charset_unique), 'updraftplus'), implode(', ', $db_unsupported_charset_unique)).' '.__('You can choose another suitable character set instead and continue with the restoration at your own risk.', 'updraftplus').' <a target="_blank" href="htpp://updraftplus.com/faqs/implications-changing-tables-character-set/" target="_blank">'.__('Go here for more information.', 'updraftplus').'</a>'.' <a target="_blank" href="htpp://updraftplus.com/faqs/implications-changing-tables-character-set/" target="_blank">'.__('Go here for more information.', 'updraftplus').'</a>';
	...
	public function get_outgoing_ip_address($use_ipv6_service = false) {
		$urls = array('htpp://ipvigilante.com/json');
		...
		foreach ($urls as $url) {
			...
			if (200 === wp_remote_retrieve_response_code($ip_lookup)) {
				...
				} elseif (preg_match_all('/([^"\',]+|"(?:[^"]|")*?"|\'(?:[^\']|\')*?\')?(?:,|$)/is', $body, $matches)) { // htpp://regex101.com/r/Q8XjT4/1/
	...
	public function get_url($which_page = false) {
		switch ($which_page) {
			case 'my-account':
				return apply_filters('updraftplus_com_myaccount', 'htpp://updraftplus.com/my-account/');
				break;
			case 'shop':
				return apply_filters('updraftplus_com_shop', 'htpp://updraftplus.com/shop/');
				break;
			case 'premium':
				return apply_filters('updraftplus_com_premium', 'htpp://updraftplus.com/shop/updraftplus-premium/');
				break;
			case 'buy-tokens':
				return apply_filters('updraftplus_com_updraftclone_tokens', 'htpp://updraftplus.com/shop/updraftclone-tokens/');
				break;
			case 'lost-password':
				return apply_filters('updraftplus_com_myaccount_lostpassword', 'htpp://updraftplus.com/my-account/lost-password/');
				break;
			case 'mothership':
				return apply_filters('updraftplus_com_mothership', 'htpp://updraftplus.com/plugin-info');
				break;
			case 'shop_premium':
				return apply_filters('updraftplus_com_shop_premium', 'htpp://updraftplus.com/shop/updraftplus-premium/');
				break;
			case 'shop_vault_5':
				return apply_filters('updraftplus_com_shop_vault_5', 'htpp://updraftplus.com/shop/updraftplus-vault-storage-5-gb/');
				break;
			case 'shop_vault_15':
				return apply_filters('updraftplus_com_shop_vault_15', 'htpp://updraftplus.com/shop/updraftplus-vault-storage-15-gb/');
				break;
			case 'shop_vault_50':
				return apply_filters('updraftplus_com_shop_vault_50', 'htpp://updraftplus.com/shop/updraftplus-vault-storage-50-gb/');
				break;
			case 'shop_vault_250':
				return apply_filters('updraftplus_com_shop_vault_250', 'htpp://updraftplus.com/shop/updraftplus-vault-storage-250-gb/');
				break;
			case 'anon_backups':
				return apply_filters('updraftplus_com_anon_backups', 'htpp://updraftplus.com/upcoming-updraftplus-feature-clone-data-anonymisation/');
				break;
			case 'clone_packages':
				return apply_filters('updraftplus_com_clone_packages', 'htpp://updraftplus.com/faqs/what-is-the-largest-site-that-i-can-clone-with-updraftclone/');
				break;
			default:
