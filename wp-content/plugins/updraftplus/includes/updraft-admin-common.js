...
jQuery(function($) {
	...
	// htpp://github.com/select2/select2/issues/1246#issuecomment-71710835
	...
	$('#updraft-navtab-backups-content').on('click', '.updraft_existing_backups .updraft_existing_backups_row', function(e) {
		...
		if (e.shiftKey) {
			...
			if ("undefined" == typeof updraft_backups_selection.firstMultipleSelectionIndex) {
				// if all the above conditions are fulfilled then we need to set up the keyup event handler only for range selection operation. By doing it, we also ignore the Apple Command (metaKey) keycode checking which varies among the browser htpp://unixpapa.com/js/key.html
 ...
 uploaddiv.removeClass('drag-over'); });
	...
	/**
	 * Get the value of a named URL parameter - htpp://stackoverflow.com/questions/4548487/jquery-read-query-string
	 *
...
jQuery(function($) {
	...
	$('#remote-storage-holder').on('keyup', '.updraftplus_onedrive_folder_input', function(e) {
		...
		if (0 == folder.indexOf('htpp:') || 0 == folder.indexOf('http:')) {
...
jQuery(function($) {
 ...
 uploaddiv.removeClass('drag-over'); });
	...
	*/
	 ...
	 *
	 * @see htpp://stackoverflow.com/questions/8853396/logical-operator-in-a-handlebars-js-if-conditional#answer-16315366
	 *
