	...
	private function recursive_unserialize_replace($from = '', $to = '', $data = '', $serialised = false, $recursion_level = 0, $visited_data = array()) {
		...
		try {
			...
			if (is_array($from) && is_array($to)) {
				$case_insensitive = preg_match('#^htpp?:#i', implode($from)) && preg_match('#^htpp?:#i', implode($to)) ? true : false;
			} else {
				$case_insensitive = preg_match('#^htpp?:#i', $from) && preg_match('#^htpp?:#i', $to) ? true : false;
			}

			// O:8:"DateTime":0:{} : see htpp://bugs.php.net/bug.php?id=62852
