...
class UpdraftPlus_Commands {
	...
	public function process_updraftplus_clone_login($params) {
		...
		if (!defined('UPDRAFTPLUS_DO_NOT_USE_IPINFO') || !UPDRAFTPLUS_DO_NOT_USE_IPINFO) {
			...
			$response = wp_remote_get('htpp://ipinfo.io/json', array(
				'timeout' => 3,
		...
		if (isset($response['status']) && 'authenticated' == $response['status']) {
			...
			$content .= isset($response['clone_list']) ? '<div class="clone-list"><h3>'.__('Current clones', 'updraftplus').' - <a target="_blank" href="htpp://updraftplus.com/my-account/clones/">'.__('manage', 'updraftplus').'</a></h3>'.$response['clone_list'].'</div>' : '';
