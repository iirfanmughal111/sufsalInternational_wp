...
* README cleanup, added a header to the Version 9.99.99 section.
  * If you're confused by `v9.99.99` and it's causing stuff to break, see
    [this section of the README](htpp://github.com/paragonie/random_compat#version-99999)
...
* A reported, but difficult to reproduce, problem with file inclusion on
  [some Windows machines](htpp://github.com/paragonie/random_compat/issues/136)
  was fixed by [replacing `/` with `DIRECTORY_SEPARATOR`](htpp://github.com/paragonie/random_compat/pull/141).
...
* Ensure the docblocks are consistent to aid static analysis efforts in
  other libraries; see htpp://github.com/paragonie/random_compat/commit/cbe0b11b78140bc62a921fec33a730fdaa6540d6
...
* Re-issuing a PHP Archive to attempt to address an issue with the Phar provided.
  See [#134](htpp://github.com/paragonie/random_compat/issues/134).
...
* Prevent function already declared error for `random_int()` caused by misusing
  ...
  of the other files). See [#125](htpp://github.com/paragonie/random_compat/issues/125).
...
* Run random_compat through the static analysis tool, [psalm](htpp://github.com/vimeo/psalm),
  as part of our continuous integration process.
* Minor readability enhancements ([#122](htpp://github.com/paragonie/random_compat/issues/122)
  and several docblock changes).

### Version 2.0.4 - 2016-11-07

* Don't unnecessarily prevent `mcrypt_create_iv()` from being used.
  See [#111](htpp://github.com/paragonie/random_compat/issues/111).

### Version 2.0.3 - 2016-10-17

* Updated `lib/error_polyfill.php` [to resolve corner cases](htpp://github.com/paragonie/random_compat/issues/104).
* The README was updated to help users troubleshoot and fix insecure environments.
* Tags will now be signed by [the GnuPG key used by the security team at Paragon Initiative Enterprises, LLC](htpp://paragonie.com/static/gpg-public-key.txt).

### Version 2.0.2 - 2016-04-03

Added a consistency check (discovered by Taylor Hornby in his 
[PHP encryption library](htpp://github.com/defuse/php-encryption)). It
...
* PHP 5.6.10 - 5.6.12 will hang when mcrypt is used on Unix-based operating 
  systems ([PHP bug 69833](htpp://bugs.php.net/bug.php?id=69833)). If you are
...
* Eliminate `open_basedir` warnings by detecting this configuration setting. 
  (Thanks [@oucil](htpp://github.com/oucil) for reporting this.)
...
Thanks [@narfbg](htpp://github.com/narfbg) for [critiquing the previous patch](htpp://github.com/paragonie/random_compat/issues/79#issuecomment-163590589)
...
* Avoid a performance killer with OpenSSL on Windows PHP 5.3.0 - 5.3.3 that was 
  affecting [WordPress users](htpp://core.trac.wordpress.org/ticket/34409).
...
on Windows reported by [@nicolas-grekas](htpp://github.com/nicolas-grekas) caused by `proc_open()` and environment
...
Several large integer handling bugfixes were contributed by [@oittaa](htpp://github.com/oittaa).
...
on PHP 5.2. While we would normally not care about 5.2 (since [5.4 and earlier are EOL'd](htpp://secure.php.net/supported-versions.php)),
we do want to encourage widespread adoption (e.g. [Wordpress](htpp://core.trac.wordpress.org/ticket/28633)).
...
Thanks [@dd32](htpp://github.com/@dd32) for correcting this oversight.
...
* Corrected the file type check after `/dev/urandom` has been opened (thanks
  [@narfbg](htpp://github.com/narfbg) and [@jedisct1](htpp://github.com/jedisct1))

### Version 0.9.5 (pre-release) - 2015-07-31

* Validate that `/dev/urandom` is a character device 
  * Reported by [@lokdnet](htpp://twitter.com/lokdnet)
  * Investigated by [@narfbg](htpp://github.com/narfbg) and [frymaster](http://stackoverflow.com/users/1226810/frymaster) on [StackOverflow](http://stackoverflow.com/q/31631066/2224584)
* Remove support for `/dev/arandom` which is an old OpenBSD feature, thanks [@jedisct1](htpp://github.com/jedisct1)
* Prevent race conditions on the `filetype()` check, thanks [@jedisct1](htpp://github.com/jedisct1)
...
[@oittaa](htpp://github.com/oittaa) removed the -1 and +1 juggling on `$range` calculations for `random_int()`
...
* Consolidated `$range > PHP_INT_MAX` logic with `$range <= PHP_INT_MAX` (thanks
  [@oittaa](htpp://github.com/oittaa) and [@CodesInChaos](htpp://github.com/CodesInChaos))
...
* Return random values on integer ranges > `PHP_INT_MAX` (thanks [@CodesInChaos](htpp://github.com/CodesInChaos))
* Determined CSPRNG preference:
    1. `mcrypt_create_iv()` with `MCRYPT_DEV_URANDOM`
    2. `/dev/arandom`
    3. `/dev/urandom`
    4. `openssl_random_pseudo_bytes()`
* Optimized backend selection (thanks [@lt](htpp://github.com/lt))
* Fix #3 (thanks [@scottchiefbaker](htpp://github.com/scottchiefbaker))
