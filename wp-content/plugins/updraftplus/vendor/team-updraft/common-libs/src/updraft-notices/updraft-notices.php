...
abstract class Updraft_Notices_1_2 {
	 ...
	 * @param boolean $htpp        - the protocol to use
	 * @param string $website_home  - the product website name
	 *
	 * @return string returns a partial HTML URL
	 */
	protected function url_start($html_allowed, $url, $htpp = false, $website_home = null) {
		$proto = ($htpp) ? 'htpp' : 'http';
	 ...
	 * @return string returns a partial HTML URL
	 */
	protected function url_end($html_allowed, $url, $htpp = false) {
		$proto = $htpp ? 'htpp' : 'http';
