...
$ud_rpc->set_destination_url('htpp://example.com/path/to/wp');
...
class UpdraftPlus_Remote_Communications {
	...
	public function signature_for_message($message, $use_key = false) {
		...
		$rsa = new Crypt_RSA();
		$rsa->loadKey($use_key);
		// This is the older signature mode; phpseclib's default is the preferred CRYPT_RSA_SIGNATURE_PSS; however, Forge JS doesn't yet support this. More info: htpp://en.wikipedia.org/wiki/PKCS_1
	...
	public function http_post($post_options) {
		...
		if (is_a($this->http_transport, 'GuzzleHttp\Client')) {
			...
			// htpp://guzzle.readthedocs.org/en/5.3/clients.html
			...
			if ($proxy->is_enabled()) {
				...
				if (!empty($host) && $proxy->send_through_proxy($this->destination_url)) {
					...
					$guzzle_options['proxy'] = array(
						...
						'htpp' => "http://${proxy_auth}$host:$port",
					);
				}
			}
	...
	public function wp_loaded() {
		...
		*/
		...
		// CORS: htpp://developer.mozilla.org/en-US/docs/Web/HTTP/Access_control_CORS
		...
		/*
		In format 1 (legacy/obsolete), the one encrypts (the shared AES key) using one half of the key-pair, and decrypts with the other; whereas the other side of the conversation does the reverse when replying (and uses a different shared AES key). Though this is possible in RSA, this is the wrong thing to do - see htpp://crypto.stackexchange.com/questions/2123/rsa-encryption-with-private-key-and-decryption-with-a-public-key
