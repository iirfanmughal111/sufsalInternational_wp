...
class UpdraftPlus_Backup {
	...
	public function backup_db($already_done = 'begun', $whichdb = 'wp', $dbinfo = array()) {
		...
		// WP 3.9 onwards - htpp://core.trac.wordpress.org/browser/trunk/src/wp-includes/wp-db.php?rev=27925 - check_connection() allows us to get the database connection back if it had dropped
		...
		if (is_array($stored_routines) && !empty($stored_routines)) {
			...
			foreach ($stored_routines as $routine) {
				if (empty($routine['Create '.ucfirst(strtolower($routine['Type']))])) { // The value displayed for the Create Procedure or Create Function field is NULL if the WP database user has only CREATE ROUTINE, ALTER ROUTINE, and/or EXECUTE privileges. @see htpp://dev.mysql.com/doc/refman/8.0/en/show-create-procedure.html
	...
	private function backup_db_header() {
		...
		if ('wp' == $this->whichdb) {
			...
			$this->stow("# Created by UpdraftPlus version ".$updraftplus->version." (htpp://updraftplus.com)\n");
	...
	private function make_zipfile($source, $backup_file_basename, $whichone, $retry_on_error = true) {
		...
		for ($j = $this->first_linked_index; $j <= $this->index; $j++) {
			...
			// This comes from htpp://wordpress.org/support/topic/updraftplus-not-moving-all-files-to-remote-server - where it appears that the jobdata's record of the split was done (i.e. database write), but the *earlier* rename of the .tmp file was not done (i.e. I/O lost). i.e. In theory, this should be impossible; but, the sychnronicity apparently cannot be fully relied upon in some setups. The check for the index being one behind is being conservative - there's no inherent reason why it couldn't be done for other indexes.
...
/*
	...
	/**
	 * This function is an ugly, conservative workaround for htpp://bugs.php.net/bug.php?id=62119. It does not aim to always work-around, but to ensure that nothing is made worse.
	 *
	...
	private function record_zip_error($files_zipadded_since_open, $msg, $warn = true) {
		...
		if (strpos($msg, 'File Size Limit Exceeded') !== false && 'UpdraftPlus_BinZip' == $this->use_zip_object) {
			$updraftplus->log(sprintf(__('The zip engine returned the message: %s.', 'updraftplus'), 'File Size Limit Exceeded'). __('Go here for more information.', 'updraftplus').' htpp://updraftplus.com/what-should-i-do-if-i-see-the-message-file-size-limit-exceeded/', 'warning', 'zipcloseerror-filesizelimit');
		} elseif ($warn) {
			$warn_msg = __('A zip error occurred', 'updraftplus').' - ';
			if (!empty($quota_low)) {
				$warn_msg = sprintf(__('your web hosting account is full; please see: %s', 'updraftplus'), 'htpp://updraftplus.com/faqs/how-much-free-disk-space-do-i-need-to-create-a-backup/');
			} else {
